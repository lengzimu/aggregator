...
**巡检的平台差异（2026-09-25 实测，`scripts/check-links.js` 已按此实现三态分类）**：

| 平台 | 自动判定 | 原因 |
|---|---|---|
| 漫画/小说等普通站 | 404/410 → 自动下架；403/5xx/超时 → 保留 | 状态码可信 |
| TikTok | 走公开 oembed 接口：404 → 下架，200 → 存活，403/网络失败 → 保留 | 站内页面对删除视频也返 200，不可信；本地沙箱整域不通，CI 海外 IP 正常 |
| 抖音 | **一律保留，人工复核** | GET 任何视频（含已下架）都返 200 风控壳；HEAD 连正常视频也 404——状态码两个方向都不可信 |

抖音条目下架后巡检报告会列在「无法自动核验」清单里，人工打开链接确认后再手动把 JSON 移入 `removed/`（封面文件会被 `removeCover` 一并清理）。
