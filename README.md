...
> ⚠️ **核心定位：链接索引服务 + 短视频封面本地存储**
> 本站**除短视频封面外不存储任何媒体文件**（视频 / 图片 / 正文），仅通过外链引用与 302 跳转把用户导流到原站。短视频封面因平台防盗链与链接时效问题，由本站本地存储——仓库只记录封面链接地址，二进制随站点部署。
...
- `coverUrl`：漫画/小说填**原站外链**（失效自动回退 `default-cover.svg`）；**短视频封面必须本地存储**——把图放入 `public/covers/videos/<slug>.jpg`，此处填站点内链接 `/covers/videos/<slug>.jpg`。缺省时**整字段省略**，不要写空串。详见 [`docs/THIRD_PARTY_PUSH.md`](./docs/THIRD_PARTY_PUSH.md)。
...
- 短视频封面由本站本地存储（放入 `public/covers/videos/`，`coverUrl` 记站点内链接）；漫画/小说封面引用原站外链，`referrerpolicy="no-referrer"`
