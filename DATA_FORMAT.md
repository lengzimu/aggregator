...
| `platform` | ✓ | ✓ | ✓ | 必须在白名单 |
| `sourceUrl` | ✓ | ✓ | ✓ | 跳转目标（原站长链） |
| `coverUrl` | **短视频必填** | 可选 | 可选 | 漫画/小说填原站外链，失效回退 `default-cover.svg`；**短视频封面必须存储**：配 R2 填 R2 公开 URL，未配 R2 填 `/covers/videos/<slug>.jpg`（见下文「短视频封面存储」）。整字段删除，不置空串 |
| `language` | ✓ | ✓ | ✓ | zh / en |
| `tags` | ✓（可空） | ✓ | ✓ | 短视频允许空数组 |
...
## 五之二、短视频封面存储（唯一存储例外，自动降级）

短视频平台的封面普遍带防盗链、原始链接易失效，因此**短视频封面必须存储**，漫画/小说封面仍走原站外链。存储后端**自动降级**：

- **配了 R2** → 封面 PUT 到 R2，对象键 `videos/<slug>.<ext>`，`coverUrl` 填 R2 公开 URL `https://<bucket>.r2.dev/videos/<slug>.<ext>`（或自定义域）。二进制不进 git 历史。
- **未配 R2** → 封面落到仓库 `public/covers/videos/<slug>.<ext>`，`coverUrl` 填站点内相对路径 `/covers/videos/<slug>.jpg`，二进制随站点部署。
- `<slug>` 与对应 JSON 文件名一致，便于追踪生命周期。
- 统一入口 `scripts/lib/r2.mjs` 的 `storeCover()` 负责选择；`node scripts/store-cover.mjs <图> <slug> --apply` 即可（无 R2 凭据时自动落仓库）。
- `coverUrl` 不要填抖音/TikTok 原站外链（会被防盗链拦截）；也不要塞进 `src/content/`。
- 第三方推送的完整规范、字段表、示例、上传方式见 **[`docs/THIRD_PARTY_PUSH.md`](./docs/THIRD_PARTY_PUSH.md)** 第 4 节。

> 定位修正：本站不再是「纯链接索引服务」——它是「链接索引服务 + 短视频封面存储」。除短视频封面外，不存储任何媒体文件。

> 为什么推荐 R2 而不是只用仓库内 `public/covers`：Cloudflare Pages 单次部署上限 **20,000 文件**，且封面进 git 历史会膨胀仓库（GitHub 推荐 < 1 GB）；R2 免费 10 GB、出网费为零、S3 兼容。但规模小（几百张内）时仓库内零配置零信用卡，完全够用。
